...
fn test_keep_all_elements() {
    ...
    let retainer = OrderedConcurrentScan::new([&mut vec1, &mut vec2], || 0);
    retainer.retain_mut(&|it| *it == 0, |_arr| [true, true]);
...
fn test_keep_only_some_shallow_eq() {
    ...
    let mut vec2 = vec![2, 3, 5, 6];
    let retainer = OrderedConcurrentScan::new([&mut vec1, &mut vec2], || 0);
    retainer.retain_mut(&|it| *it == 0, |arr| [*arr[0] == 3, *arr[1] == 3]);
    // Shallow_eq sets: [2,2], [3,3]
...
fn test_remove_all_shallow_eq() {
    ...
    let mut vec2 = vec![2, 3, 5, 6];
    let retainer = OrderedConcurrentScan::new([&mut vec1, &mut vec2], || 0);
    retainer.retain_mut(&|it| *it == 0, |_arr| [false, false]);
    // Shallow_eq sets: [2,2], [3,3]
