...
impl ProjectOrthogonallyOnto<MultiVector> for Flector {
    ...
    fn project_orthogonally_onto(self, other: MultiVector) -> Self::Output {
        ...
        let wedge_g0_y = (self[e1] * other[e1]) + (self[e2] * other[e2]) + (self[e3] * other[e3]) - (right_anti_dual_g1_w * self[e321]);
...
impl ProjectOrthogonallyOnto<Flector> for MultiVector {
    ...
    fn project_orthogonally_onto(self, other: Flector) -> Self::Output {
        use crate::elements::*;
        let right_anti_dual_g0_w = other[e321] * -1.0;
        let wedge_g0_y = (other[e1] * self[e1]) + (other[e2] * self[e2]) + (other[e3] * self[e3]) - (right_anti_dual_g0_w * self[e321]);
        ...
        let wedge_g4_xyz = (Simd32x3::from(right_anti_dual_g0_w) * self.group3()) + (Simd32x3::from(self[scalar]) * other.group0().xyz());
        MultiVector::from_groups(
            // scalar, e1234
            Simd32x2::from([
...
impl ProjectOrthogonallyOnto<MultiVector> for MultiVector {
    ...
    fn project_orthogonally_onto(self, other: MultiVector) -> Self::Output {
        ...
        let wedge_g4 = (Simd32x3::from(right_anti_dual_g1_w) * self.group3()).with_w(0.0)
