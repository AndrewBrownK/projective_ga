...
impl ProjectOrthogonallyOnto<Flector> for Flector {
    ...
    fn project_orthogonally_onto(self, other: Flector) -> Self::Output {
        ...
        let wedge_g0_w = (other[e1] * self[e1]) - (right_anti_dual_g0_w * self[e321]);
        Flector::from_groups(
            // e1, e2, e3, e4
            ((Simd32x3::from(wedge_g0_w) * other.group0().xyz()) - (Simd32x3::from(right_anti_dual_g0_w * other[e321]) * self.group0().xyz())).with_w(wedge_g0_w * other[e4]),
            // e423, e431, e412, e321
...
impl ProjectOrthogonallyOnto<MultiVector> for Flector {
    ...
    fn project_orthogonally_onto(self, other: MultiVector) -> Self::Output {
        ...
        let wedge_g2 = Simd32x3::from(right_anti_dual_g1_w * -1.0) * self.group0().xyz();
...
impl ProjectOrthogonallyOnto<Flector> for Motor {
    ...
    fn project_orthogonally_onto(self, other: Flector) -> Self::Output {
        ...
        Motor::from_groups(
            ...
            // e23, e31, e12, scalar
            (wedge_g1_xyz * Simd32x4::from(other[e321]).xyz() * Simd32x3::from(-1.0)).with_w((wedge_g1_xyz[0] * other[e1]) - (right_anti_dual_g0_w * other[e321] * self[scalar])),
        )
    }
}
...
impl ProjectOrthogonallyOnto<MultiVector> for Motor {
    ...
    fn project_orthogonally_onto(self, other: MultiVector) -> Self::Output {
        ...
        let right_anti_dual_g1_w = other[e321] * -1.0;
        let right_anti_dual_g2 = other.group3() * Simd32x3::from(-1.0);
        let wedge_g0_y = (other[scalar] * self[scalar]) - (right_anti_dual_g2[0] * self[e23]) - (right_anti_dual_g2[1] * self[e31]) - (right_anti_dual_g2[2] * self[e12]);
...
impl ProjectOrthogonallyOnto<Plane> for Motor {
    ...
    fn project_orthogonally_onto(self, other: Plane) -> Self::Output {
        ...
        Motor::from_groups(
            ...
            // e23, e31, e12, scalar
            (wedge_g1_xyz * Simd32x4::from(other[e321]).xyz() * Simd32x3::from(-1.0)).with_w(right_anti_dual_g0 * self[scalar] * other[e321] * -1.0),
        )
    }
}
...
impl ProjectOrthogonallyOnto<Flector> for MultiVector {
    ...
    fn project_orthogonally_onto(self, other: Flector) -> Self::Output {
        ...
        MultiVector::from_groups(
            // scalar, e1234
            Simd32x2::from([
                (wedge_g4_xyz[0] * other[e1]) + (wedge_g4_xyz[1] * other[e2]) + (wedge_g4_xyz[2] * other[e3]) - (right_anti_dual_g0_w * self[scalar] * other[e321]),
                0.0,
            ]),
            // e1, e2, e3, e4
            ((Simd32x3::from(wedge_g0_y) * other.group0().xyz()) - (Simd32x3::from(right_anti_dual_g0_w * other[e321]) * self.group1().xyz())).with_w(wedge_g0_y * other[e4]),
            // e41, e42, e43
...
impl ProjectOrthogonallyOnto<MultiVector> for MultiVector {
    ...
    fn project_orthogonally_onto(self, other: MultiVector) -> Self::Output {
        ...
        let right_anti_dual_g2 = other.group3() * Simd32x3::from(-1.0);
        let right_anti_dual_g4_xyz = other.group1().xyz();
        let wedge_g0_y = (other[scalar] * self[scalar]) + (right_anti_dual_g4_xyz[0] * self[e1]) + (right_anti_dual_g4_xyz[1] * self[e2]) + (right_anti_dual_g4_xyz[2] * self[e3])
        ...
        let wedge_g4 =
            ((right_anti_dual_g4_xyz * Simd32x3::from(self[scalar])) + (Simd32x3::from(right_anti_dual_g1_w) * self.group3()) + (right_anti_dual_g2.yzx() * self.group1().zxy())
...
impl ProjectOrthogonallyOnto<Plane> for MultiVector {
    ...
    fn project_orthogonally_onto(self, other: Plane) -> Self::Output {
        ...
        MultiVector::from_groups(
            ...
            Simd32x2::from([right_anti_dual_g0 * self[scalar] * other[e321] * -1.0, 0.0]),
            // e1, e2, e3, e4
            (Simd32x3::from(right_anti_dual_g0 * -1.0) * Simd32x4::from(other[e321]).xyz() * self.group1().xyz()).with_w(0.0),
            // e41, e42, e43
...
impl ProjectOrthogonallyOnto<Point> for MultiVector {
    ...
    fn project_orthogonally_onto(self, other: Point) -> Self::Output {
        use crate::elements::*;
        let right_anti_dual_g0_xyz = other.group0().xyz();
        let wedge_g4 = Simd32x4::from([1.0, 1.0, self[scalar], 0.0]) * (right_anti_dual_g0_xyz * Simd32x2::from(self[scalar]).with_z(1.0)).with_w(0.0);
        MultiVector::from_groups(
            // scalar, e1234
