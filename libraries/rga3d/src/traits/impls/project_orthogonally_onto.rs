...
impl ProjectOrthogonallyOnto<Flector> for Motor {
    ...
    fn project_orthogonally_onto(self, other: Flector) -> Self::Output {
        use crate::elements::*;
        let right_anti_dual_g0 = Simd32x3::from(0.0).with_w(other[e321]) * Simd32x4::from([0.0, 0.0, 0.0, -1.0]);
        let wedge_g1 = (Simd32x3::from(right_anti_dual_g0[3]) * self.group1().xyz()).with_w(0.0)
...
impl ProjectOrthogonallyOnto<MultiVector> for Motor {
    ...
    fn project_orthogonally_onto(self, other: MultiVector) -> Self::Output {
        ...
        let wedge_g1 = Simd32x3::from(0.0).with_w(right_anti_dual_g1[3] * self[scalar]);
        let wedge_g2 = right_anti_dual_g2 * Simd32x3::from(self[scalar]);
        let wedge_g4 = (Simd32x3::from(right_anti_dual_g1[3]) * self.group1().xyz()).with_w(0.0)
...
impl ProjectOrthogonallyOnto<Flector> for MultiVector {
    ...
    fn project_orthogonally_onto(self, other: Flector) -> Self::Output {
        ...
        let wedge_g0_y = (other[e1] * self[e1]) + (other[e2] * self[e2]) + (other[e3] * self[e3])
            ...
            - (right_anti_dual_g0[2] * self[e412])
            - (right_anti_dual_g0[3] * self[e321]);
        let wedge_g2 = (Simd32x3::from(self[e4]) * right_anti_dual_g0.xyz()) - (Simd32x3::from(right_anti_dual_g0[3]) * self.group1().xyz());
        let wedge_g4 = (Simd32x3::from(right_anti_dual_g0[3]) * self.group3()).with_w(0.0)
            + (Simd32x3::from(self[scalar]) * other.group0().xyz()).with_w(0.0)
        ...
        return MultiVector::from_groups(
            ...
            Simd32x2::from([
                (wedge_g4[0] * other[e1]) + (wedge_g4[1] * other[e2]) + (wedge_g4[2] * other[e3]) + (wedge_g4[3] * other[e4])
                    - (right_anti_dual_g0[3] * other[e321] * self[scalar]),
                0.0,
            ]),
            // e1, e2, e3, e4
...
impl ProjectOrthogonallyOnto<Horizon> for MultiVector {
    ...
    fn project_orthogonally_onto(self, other: Horizon) -> Self::Output {
        ...
        return MultiVector::from_groups(
            ...
            // e1, e2, e3, e4
            (Simd32x3::from(right_anti_dual_g0) * Simd32x3::from(other[e321]) * self.group1().xyz() * Simd32x3::from(-1.0)).with_w(0.0),
            // e41, e42, e43
            Simd32x3::from(0.0),
            // e23, e31, e12
...
impl ProjectOrthogonallyOnto<MultiVector> for MultiVector {
    ...
    fn project_orthogonally_onto(self, other: MultiVector) -> Self::Output {
        ...
        let wedge_g0_y = (other[scalar] * self[scalar]) + (other[e1] * self[e1]) + (other[e2] * self[e2]) + (other[e3] * self[e3])
            ...
            - (right_anti_dual_g1[3] * self[e321]);
        let wedge_g1 = Simd32x3::from(0.0).with_w(right_anti_dual_g1[3] * self[scalar]);
        let wedge_g2 = (right_anti_dual_g2 * Simd32x3::from(self[scalar])) + (Simd32x3::from(self[e4]) * right_anti_dual_g1.xyz())
            - (Simd32x3::from(right_anti_dual_g1[3]) * self.group1().xyz());
        let wedge_g4 = (Simd32x3::from(right_anti_dual_g1[3]) * self.group3()).with_w(0.0)
            + (Simd32x3::from(self[scalar]) * other.group1().xyz()).with_w(0.0)
