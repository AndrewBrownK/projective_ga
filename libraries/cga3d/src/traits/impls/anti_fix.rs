...
impl AntiFix for AntiFlatPoint {
    ...
    fn anti_fix(self) -> Self {
        ...
        AntiFlatPoint::from_groups(
            // e235, e315, e125, e321
            (self.group0().xyz() * Simd32x3::from(-1.0) / Simd32x4::from(self[e321]).xyz()).with_w(-1.0),
        )
    }
}
...
impl AntiFix for FlatPoint {
    ...
    fn anti_fix(self) -> Self {
        use crate::elements::*;
        FlatPoint::from_groups(/* e15, e25, e35, e45 */ (self.group0().xyz() / Simd32x4::from(self[e45]).xyz()).with_w(1.0))
    }
}
